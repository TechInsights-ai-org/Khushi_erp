...
def get_columns() -> list[dict]:
    ...
    return [
        ...
        {"label": "Subcontract Qty To Receive", "fieldname": "subcontract_qty_to_receive", "fieldtype": "float"}]
...
def get_select_field(filters: dict) -> str:
    ...
    so.qty as "Qty to Deliver", COALESCE(b.actual_qty, 0) AS "stock_qty",
    COALESCE(poiq.poi_qty, 0) AS "poi_qty", COALESCE(scirq.subcontract_qty, 0) AS "subcontract_qty_to_receive" """
    ...
    if group_by_item:
        ...
        GROUP_CONCAT(DISTINCT so.customer) as "Customer", so.item_code as "Item Code", SUM(so.qty) as "Qty to Deliver",
        COALESCE(b.actual_qty, 0) AS "stock_qty", COALESCE(poiq.poi_qty, 0) AS "poi_qty", 
        COALESCE(scirq.subcontract_qty, 0) AS "subcontract_qty_to_receive" """
...
def get_poiq_query() -> str:
    ...
    poiq_query: str = """SELECT sum(poiq.poi_qty) AS poi_qty, poiq.item_code FROM
...
def get_scirq_query():
    ...
    scirq_query: str = """SELECT SUM(scirq.subcontract_qty) AS subcontract_qty, scirq.item_code AS item_code FROM
            `veuSubcontracting Item Receive Quantity` AS scirq GROUP BY scirq.item_code"""
    return scirq_query
